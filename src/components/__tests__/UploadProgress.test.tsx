...
describe('UploadProgress Component', () => {
  ...
  it('renders correct status message and icon for each state (idle, uploading, completed, error)', async () => {
    const { rerender } = render(<UploadProgress fileName="test.txt" progress={0} status="idle" />);
    expect(screen.getByText('Preparing to upload...')).toBeInTheDocument();
    expect(screen.getAllByRole('progressbar')[1]).toBeInTheDocument();

    await act(async () => {
      rerender(<UploadProgress fileName="test.txt" progress={50} status="uploading" />);
    });
    expect(screen.getByText('Uploading your message...')).toBeInTheDocument();

    await act(async () => {
      rerender(<UploadProgress fileName="test.txt" progress={100} status="completed" />);
    });
    expect(screen.getByText('Upload complete!')).toBeInTheDocument();

    await act(async () => {
      rerender(<UploadProgress fileName="test.txt" progress={0} status="error" error="Upload failed" />);
    });
  ...
  it('renders ProgressBar with correct progress and variant', async () => {
    const { rerender } = render(<UploadProgress fileName="test.txt" progress={50} status="uploading" />);
  ...
  it('renders ErrorModal when error prop is present', async () => {
    const { rerender } = render(<UploadProgress fileName="test.txt" progress={0} status="error" error="Upload failed" />);
    await act(async () => {
      rerender(<UploadProgress fileName="test.txt" progress={0} status="error" error="Upload failed" />);
    });
  ...
  it('renders "Continue" button when status is "completed"', () => {
    render(<UploadProgress fileName="test.txt" progress={100} status="completed" onComplete={() => {}} />)
    expect(screen.getByText('Continue')).toBeInTheDocument()
  })

  it('does not render "Continue" button when status is not "completed"', () => {
    render(<UploadProgress fileName="test.txt" progress={50} status="uploading" />)
    expect(() => screen.getByText('Continue')).toThrowError()
  })

  it('calls onRetry when "Retry Upload" button is clicked', async () => {
    const onRetry = vi.fn();
    render(<UploadProgress fileName="test.txt" progress={0} status="error" error="Upload failed" onRetry={onRetry} />);
    await act(async () => {
  ...
  it('calls onComplete when "Continue" button is clicked', async () => {
    ...
    render(<UploadProgress fileName="test.txt" progress={100} status="completed" onComplete={onComplete} />);
    await act(async () => {
